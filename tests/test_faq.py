...
def no_mongo_in_faq_tests(monkeypatch):
    # Optional provider tests exercise LLM retrieval, never a production chat database.
    monkeypatch.setattr("iai.app.graph.iniciar_sessao", lambda *a, **k: None)
    monkeypatch.setattr("iai.app.graph.salvar_mensagem", lambda *a, **k: None)
